...
threshold.contact<-function(randomangle="close"){
  ...
  rho<-sample(distances,points,replace=TRUE) #get wide range of distances
  ...
  if (randomangle=="close"){
    ...
    phi[rho<=threshold]<- rtriangle(n = length(phi[rho<=threshold]), a = pi/4, b = 3*pi/4, c = pi/2) #runif(length(phi[rho>1]), 0, pi)
    
    theta[rho>threshold]<-0
    theta[rho<=threshold]<-runif(length(theta[rho<=threshold]),0,2*pi)
  ...
  frame<-data.frame(x=x,y=y,z=z,C=C,rho=rho,theta=theta,phi=phi,Dose=Dose,
                    omega.y=omega.y,omega.z=omega.z,Q.U=Q/U,I=I,X=X,A=A,risk=risk)
  ...
  frame.sum<-data.frame(mean=c(dose.mean,infection.mean),sd=c(dose.sd,infection.sd),distance=distances,param=c(rep("dose",length(distances)),rep("risk",length(distances))))
  
  frametemp<-data.frame(dose=dose.mean,risk=infection.mean,distance=distances,threshold=threshold,randomangle=randomangle)
...
threshold.contact(randomangle = "close")
frame.close<-frame

threshold.contact(randomangle="far")
frame.far<-frame

frame<-rbind(frame.close,frame.far)
...
ggplot(data=frame)+
  ...
  geom_point(aes(x=distance,y=dose,color="dose",group=randomangle,alpha=randomangle),size=5)+
  ...
  theme(axis.text = element_text(size=18),axis.title = element_text(size=20),legend.text = element_text(size=20),strip.text=element_text(size=18),
        legend.title=element_text(size=20),legend.direction = "horizontal")+guides(alpha=FALSE)
